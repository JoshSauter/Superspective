...
Shader "Hidden/Wilberforce/Colorbleed"
{
	...
	CGINCLUDE
		...
		fixed4 bleedRaymarch(v2fShed i, int isDoomMode, int isAdaptive, int UseGBuffer, float power, float4 kernel[70], float kernelWeight, int kernelLength, int raymarchSteps, int randomizeLengths)
		{
			...
			if (isDoomMode != 0) {
				...
				doomness += tex2Dlod(doomTexture, float4(i.uv + float2(-_tsizedoom.x, -_tsizedoom.y), 0, 0)).rgba;
				doomness += tex2Dlod(doomTexture, float4(i.uv + float2(_tsizedoom.x, -_tsizedoom.y), 0, 0)).rgba;
				if (isDoomMode == 1 && doomness.r > 3.9f && doomness.g > 3.9f && doomness.b > 3.9f) return float4(1.0f, 1.0f, 1.0f, 1.0f);
			}
			...
			if (isAdaptive != 0) {
				if (isDoomMode == 2 && (doomness.r > 3.9f && doomness.g > 3.9f && doomness.b > 3.9f)) {
					start = 48;
					kernelLength = 8;
				} else {
			...
			} else if (isDoomMode == 2 && (doomness.r > 3.9f && doomness.g > 3.9f && doomness.b > 3.9f)) {
			...
			for (int j = start; j < start + kernelLength; j++) {
					...
					// Rotate sample and place it to hemisphere around shaded point
					...
					for (int s = 0; s < raymarchSteps; s++) {
						...
						// Recalculate view space position
						...
						if (sampleViewSpacePosition.z > sampledPointViewSpacePosition.z) {
							...
							// Move furhter along ray 
							sampleRay += sampleRayStep;

							// Continue while hit is not found
							continue;
						}
						
						// Hit was found - remove 1 added prior to this loop and continue
						gi -= 1.0f;
						...
						if (sampleDistance > radius) { 
							...
							// Set distance attenuation to "none" when we're using point cloud - points are distributed around the point in uniform distance steps and this will create same effect
							...
							// Attenuate by angle of normals
							/*if (cosineAtt == 1) */weight *= (1.0f - cosineNormals);

							// Attenuate by distance
							/*if (distanceAtt == 1) */weight *= distanceAttenuation;
								
							// Attenuate by angle to sampled surface (negative half-space rejection ???)
							weight *= max(0.0f, dot(pixelNormal, samplePointDirection));
						
							// Attenuate by angle to sampled surface (back-facing rejection ???)
							//if (giBackfaces == 1) weight *= max(0.5f, dot(-sampleNormal, samplePointDirection));
	...
	SubShader
	{
		...
		// 7 - ColorbleedRaymarchDoomPassGBuffer
		Pass{CGPROGRAM
			#pragma vertex vertShed #pragma fragment frag
