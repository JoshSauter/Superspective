...
// Wilberforce Colorbleed Unity Plugin 1.1
...
namespace Wilberforce.Colorbleed
{
    ...
    public class Colorbleed : MonoBehaviour
    {
        ...
        public DoomModeType CullingPrepassMode = DoomModeType.Careful;
        ...
        public bool ForcedSwitchPerformedGBufferDeferredSinglePassStereo = false;
        ...
        private void CreateCommandBuffer()
        {
            ...
            if (CullingPrepassMode != DoomModeType.Off)
            {
        ...
        private bool CheckSettingsChanges()
        {
            ...
            if (CullingPrepassMode != lastCullingPrepassMode)
            {
                lastCullingPrepassMode = CullingPrepassMode;
                settingsDirty = true;
            }
            ...
            if (OutputCBOnly != lastOutputCBOnly)
            {
                lastOutputCBOnly = OutputCBOnly;
                settingsDirty = true;
            }
        ...
        private void TrySetUniforms()
        {
            ...
            ColorbleedMaterial.SetInt("doomMode", (int) CullingPrepassMode);
            ...
            ColorbleedMaterial.SetFloat("cameraFarPlane", myCamera.farClipPlane);
            ColorbleedMaterial.SetInt("UseCameraFarPlane", FarPlaneSource == FarPlaneSourceType.Camera ? 1 : 0);
            ...
            {
                if (CullingPrepassMode == DoomModeType.Careful)
                {
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            RenderTexture dimensionsSource = destination;
            if (destination == null) dimensionsSource = source;
            if (this.destinationWidth != dimensionsSource.width || this.destinationHeight != dimensionsSource.height)
            {
                this.destinationWidth = dimensionsSource.width;
                this.destinationHeight = dimensionsSource.height;
    ...
    public class ColorbleedEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            Colorbleed effect = (target as Colorbleed);
            camera = effect.GetComponent<Camera>();

#if UNITY_5_6_OR_NEWER
            if (camera != null) isHDR = camera.allowHDR;
#else
            if (camera != null) isHDR = camera.hdr;
#endif
            SetIcon();

            // See if there is post processing stuck
            if (camera != null && (effect.CommandBufferEnabled == false || camera.actualRenderingPath == RenderingPath.DeferredShading))
            {
                try
                {
                    System.Reflection.Assembly asm = System.Reflection.Assembly.GetExecutingAssembly();
                    Type postStackType = asm.GetType("UnityEngine.PostProcessing.PostProcessingBehaviour");
                    var postStack = effect.GetComponent(postStackType);
                    if (postStack != null)
                    {
                        if (effect.ForcedSwitchPerformed)
                        {
                        ...
                        else
                        {
                            ...
                            effect.CommandBufferEnabled = true;
                            effect.UseGBuffer = true;
                            effect.ForcedSwitchPerformed = true;
                            effect.ForceCommandBufferRebuild();
                        }
                    }
                }
                catch (Exception) { }
                ...
                {
                    if (effect.ForcedSwitchPerformedSinglePassStereo)
                    {
                    ...
                    else
                    {
                        ...
                        effect.CommandBufferEnabled = true;
                        effect.ForcedSwitchPerformedSinglePassStereo = true;
                        effect.ForceCommandBufferRebuild();
                    }

#if UNITY_2017_1_OR_NEWER
                    if (camera.actualRenderingPath == RenderingPath.DeferredShading && !effect.UseGBuffer)
                    {
                        if (effect.ForcedSwitchPerformedGBufferDeferredSinglePassStereo)
                        {
                            ReportWarning("You are running in single pass stereo mode and DEFERRED rendering path! We recommend switching to GBuffer inputs.");
                        }
                        else
                        {
                            ReportWarning("You are running in single pass stereo mode! Switching to GBuffer inputs (recommended setting)!");
                            effect.UseGBuffer = true;
                            effect.ForcedSwitchPerformedGBufferDeferredSinglePassStereo = true;
                            effect.ForceCommandBufferRebuild();
                        }
                    }
#endif
        ...
        override public void OnInspectorGUI()
        {
            ...
            if (optimizationFoldout)
            {
                ...
                cbScript.CullingPrepassMode = (Colorbleed.DoomModeType)EditorGUILayout.EnumPopup(doomModeLabelContent, cbScript.CullingPrepassMode);
            ...
            if (pipelineFoldout)
            {
                ...
                if (cbScript.CommandBufferEnabled)
                {
                    cbScript.IntermediateScreenTextureFormat = (Colorbleed.ScreenTextureFormat)EditorGUILayout.EnumPopup(screenFormatLabelContent, cbScript.IntermediateScreenTextureFormat);
                }

                cbScript.FarPlaneSource = (Colorbleed.FarPlaneSourceType)EditorGUILayout.EnumPopup(farPlaneSourceLabelContent, cbScript.FarPlaneSource);
