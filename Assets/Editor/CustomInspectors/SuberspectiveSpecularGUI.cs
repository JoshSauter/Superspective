...
namespace UnityEditor {
    internal class SuberspectiveSpecularGUI : SuberspectiveGUI {
        private Shader _shader;
        public override Shader GetShader() {
            if (_shader == null) {
                _shader = Shader.Find("Suberspective/SuberspectiveSpecular");
            }

            return _shader;
        }

        public override PassType[] GetPassTypes() => new[] { PassType.ForwardBase, PassType.ForwardAdd };
        ...
        void SetMaterialKeywords(Material material, WorkflowMode workflowMode) {
        ...
        // Unity's Specular shader uses up all the shader variants, so for SuberspectiveSpecular we instead use
        // Shader properties to mimic this behavior (albeit in a less performant way)
        public override bool IsSuberspectiveKeywordEnabled(Material material, string keyword) {
            return material.GetFloat($"__{keyword}") > 0.0f;
        }
        
        public override void SetSuberspectiveKeyword(Material material, string keyword, bool state) {
            material.SetFloat($"__{keyword}", state ? 1 : 0);
        }
