...
// Credit to https://www.programmersought.com/article/89035922007/
...
#pragma multi_compile __ _ENABLE_SIGNAL
	...
	{
		half4 totalColor;
#if _ENABLE_SIGNAL//Signal interference effect
		...
		{
			 totalColor = totalColor * _StripeColor;//Horizontal stripes
		}
		 totalColor.rgb *= saturate(random(IN.texcoord.x, IN.texcoord.y% 0.01 * _Time.y) + (1-_SignalNoise));//noise
		...
		if (flowLightOffset < 1)
		{
			float4 flowLightColor = tex2D(_FlowLightTex, float2(IN.texcoord.x, IN.texcoord.y + 1 - flowLightOffset));
			 totalColor.rgb = totalColor.rgb + flowLightColor.rgb;//Stream Light
		}
