		...
		Pass {
			...
			*/
			...
			fixed4 Frag(UVPositions uvPositions) : SV_Target {
				...
				half verticalIsSame = 1;
				half horizontalIsSame = 1;
				half similarDepth = 1;
				...
				for (int x = 1; x < NUM_SAMPLES; x++) {
					...
					*/
					...
					if (x == 5 || x == 7) {
						verticalIsSame *= thisDepthIsSimilar * thisNormalIsSimilar;
					}
					if (x == 6 || x == 8) {
						horizontalIsSame *= thisDepthIsSimilar * thisNormalIsSimilar;
					}
				}
				...
				*/
				...
				if (verticalIsSame == 0 && horizontalIsSame == 1) {
					if (uvPositions.UVs[0].x * 300 % 2 > 1) {
						return original;
					}
				}
				if (horizontalIsSame == 0 && verticalIsSame == 1) {
					if (uvPositions.UVs[0].y * 100 % 2 > 1) {
						return original;
					}
				}
				if (horizontalIsSame == 0 && verticalIsSame == 0) {
					if (uvPositions.UVs[0].x * 300 % 2 > 1 || uvPositions.UVs[0].y * 100 % 2 > 1) {
						return original;
					}
				}
