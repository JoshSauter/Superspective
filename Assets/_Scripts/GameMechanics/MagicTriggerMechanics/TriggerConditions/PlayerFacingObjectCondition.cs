...
namespace MagicTriggerMechanics.TriggerConditions {
    [Serializable]
    public class PlayerFacingObjectCondition : ThresholdTriggerCondition {
        ...
        protected override float Evaluate(Transform triggerTransform) {
            ...
            Vector3 objectToPlayerVector = (targetObject.ClosestPointOnBounds(camPos) - camPos).normalized;
            // TODO: Handle player being inside of object as a "always false" case
            bool insideTargetObject = false;
            return insideTargetObject ? -1 : Vector3.Dot(camDirection, objectToPlayerVector);
        }
