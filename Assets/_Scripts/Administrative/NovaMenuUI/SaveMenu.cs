...
namespace NovaMenuUI {
    public class SaveMenu : NovaSubMenu<SaveMenu> {
        ...
        void Start() {
            ...
            SaveFileUtils.OnSavesChanged += () => { PopulateSaveSlots(true); };
        ...
        void InitSaveMenuStateMachine() {
            saveMenuState.OnStateChangeSimple += () => { PopulateSaveSlots(true); };

            saveMenuState.AddTrigger(enumValue => true, RunAnimationAndUpdateState);
            saveMenuState.AddTrigger(SaveMenuState.Off, fadeAnimationDuration, () => { transform.parent.localPosition = transform.parent.localPosition.WithZ(1); });
            saveMenuState.AddTrigger(enumValue => enumValue != SaveMenuState.Off, () => { transform.parent.localPosition = transform.parent.localPosition.WithZ(0); });
        }
