...
namespace NovaMenuUI {
    // Adapted from Nova's TextField component
    public class SuperspectiveTextField : MonoBehaviour {
        /// <summary>
        ...
        public TextBlock placeHolderText = null;
        
        [SerializeField]
        [Tooltip("Will set PlaceHolderText to this at start")]
        public string placeHolderMessage = "Type some text...";

        [Header("Maximum Length")]
        [SerializeField]
        [Tooltip("Whether to enforce a maximum length for the input string")]
        private bool enforceMaximumLength = false;

        [SerializeField]
        [ShowIf("enforceMaximumLength")]
        ...
        /// <summary>
        /// The <see cref="SuperspectiveTextField"/>'s current text, which may not be the same as the string
        /// that is visible (i.e. <see cref="TextBlock">TextBlock</see>.<see cref="Text">Text</see>) due to 
        /// <see cref="SuperspectiveTextField"/> needing to add empty spaces to ensure correct cursor position
        /// in certain sitations.
        /// </summary>
        /// <remarks>
        /// This should not be used internally (i.e. by <see cref="SuperspectiveTextField"/> itself), 
    ...
    #region Public API
        ...
        public virtual string SanitizeInput(string existingString, string desiredString) {
    ...
    #region Text Modification
        ...
        public void Insert(char c) {
            if (enforceMaximumLength && Text.Length >= maximumLength) {
                return;
            }
        ...
        public void Insert(string toInsert) {
            ...
            if (cursorPosition.TextMeshIsEmpty) {
                toInsert = SanitizeInput(TextBlock.Text, toInsert);
                ...
                newText = SanitizeInput(FromDisplayText(TextBlock.Text), FromDisplayText(newText));

                if (newText == FromDisplayText(TextBlock.Text)) {
                    return;
                }
        ...
        private void Delete(TextPosition left, TextPosition right) {
            ...
            newText = SanitizeInput(TextBlock.Text, newText);
    ...
    #region Display Text Conversions
        ...
        /// <summary>
        /// A cached, shared string builder that all text fields use to convert strings
        /// to and from display strings
        /// </summary>
        ...
        public static string ToDisplayText(string s) {
            ...
            for (int i = 0; i < s.Length; i++) {
                if (s[i].IsEmptySpace()) {
                    // Don't add re-add any empty spaces we may have added previously,
                    // otherwise we will end up with multiple empty spaces after newlines
                    continue;
                }
                ...
                if (s[i].IsNewline()) {
                    // If the character was a newline, add an empty space
                    sb.Append(CharExtensions.EmptyWidthSpace);
                }
            }
        ...
        /// <summary>
        /// Removes any empty spaces from <paramref name="s"/>.
        /// </summary>
