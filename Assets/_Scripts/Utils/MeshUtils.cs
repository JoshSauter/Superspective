	...
	public static class MeshUtils {
		...
		*/
		public class Edge {
			...
			public readonly int[] vertexIndex = new int[2];

			// The index into the face.
			// (faceIndex[0] == faceIndex[1] means the edge connects to only one triangle)
			public readonly int[] faceIndex = new int[2];
		}

		public static void ExtrudeMesh(Mesh srcMesh, Mesh extrudedMesh, Matrix4x4[] extrusion, Matrix4x4 transformationOnBackFace, bool invertFaces, bool removeFrontFace) {
			Edge[] edges = BuildManifoldEdges(srcMesh);
			ExtrudeMesh(srcMesh, extrudedMesh, extrusion, transformationOnBackFace, edges, invertFaces, removeFrontFace);
		}

		private static void ExtrudeMesh(Mesh srcMesh, Mesh extrudedMesh, Matrix4x4[] extrusion, Matrix4x4 transformationOnBackFace, Edge[] edges, bool invertFaces, bool removeFrontFace) {
			...
			int[] triangles = new int[extrudedTriIndexCount + inputTriangles.Length * (removeFrontFace ? 1 : 2)];
			...
			for (int j = v / 2; j < v; j++) {
				vertices[j] = transformationOnBackFace.MultiplyPoint(vertices[j]);
			}
			...
			for (int c = 0; c < 2; c++) {
				...
				for (int i = 0; i < inputVertices.Length; i++) {
					vertices[firstCapVertex + i] = transformationOnBackFace.MultiplyPoint(matrix.MultiplyPoint(inputVertices[i]));
			...
			// Bottom
			{
				...
				int firstCapTriIndex = extrudedTriIndexCount + (removeFrontFace ? 0 : inputTriangles.Length);
				for (int i = 0; i < triCount; i++) {
