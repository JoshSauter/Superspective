...
v2f NovaVert(NovaQuadVert v, uint instanceID : SV_InstanceID, uint vid : SV_VERTEXID)
{
    ...
    float2 nodePos = vert.Pos;

    float3 rootSpace = mul(transformAndLighting.RootFromBlock, float4(nodePos, 0, 1)).xyz;
    ...
    float2 sizeIncreaseMask = step(abs(borderedHalfSize - abs(nodePos)), epsilon);
    sizeIncreaseMask *= vert.EdgeSoftenMask;
    float2 scale = GetBlockFromViewScale(transformAndLighting.RootFromBlock);
    nodePos += sign(nodePos) * sizeIncreaseMask * sizeIncreaseAmount * scale;
    rootSpace = mul(transformAndLighting.RootFromBlock, float4(nodePos, 0, 1)).xyz;

    float nFactor = GetNFactor(halfBlockSize);
    float2 nHalfSize = halfBlockSize * nFactor;

    float2 nPos = nodePos * nFactor;
    SetNPos(o, nPos);
    ...
    float2 unrotatedGradientSpacePos = nodePos - shaderData.GradientCenter;
    ...
    SetGradientSpaceUV(o, gradientUV);
        ...
        float2 uv = SafeDividePositive(nodePos, halfBlockSize);
...
fixed4 NovaFrag(v2f i) : SV_Target
{
    ...
    fixed4 color = lerp(GetGradientColor(i), GetColor(i), min(1.0, gradientLerpVal));
        ...
        half2 softenInverse = GetSoftenWidthInverse2(nPositions);
        ...
        half softenInverse = GetSoftenWidthInverse(GetNPos(i));
