...
:: Replace placeholder in depot VDF
echo Creating windows_depot.vdf...
type "%SCRIPT_DIR%windows_depot.template.vdf" > "%SCRIPT_DIR%windows_depot.vdf"
powershell -Command "(Get-Content '%SCRIPT_DIR%windows_depot.vdf').Replace('__BUILD_PATH__', '%FULL_BUILD_PATH%') | Set-Content '%SCRIPT_DIR%windows_depot.vdf'"
echo windows_depot.vdf complete!

:: Replace placeholder in app build VDF
echo Creating app_build.vdf...
type "%SCRIPT_DIR%app_build.template.vdf" > "%SCRIPT_DIR%app_build.vdf"
powershell -Command "(Get-Content '%SCRIPT_DIR%app_build.vdf').Replace('__VERSION_DESC__', '%VERSION_DESC%') | Set-Content '%SCRIPT_DIR%app_build.vdf'"
echo app_build.vdf complete!

:: Run SteamCMD upload using script-relative path
echo Running Steam upload (you may be prompted to login)
C:\steamcmd\steamcmd.exe +login goshjosh182 +run_app_build "%SCRIPT_DIR%app_build.vdf" +quit
